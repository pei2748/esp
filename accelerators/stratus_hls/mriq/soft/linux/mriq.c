...
static inline struct mriq_device *to_mriq(struct esp_device *esp)
{
	return container_of(esp, struct mriq_device, esp);
}

static void mriq_prep_xfer(struct esp_device *esp, void *arg)
{
	struct mriq_access *a = arg;
...
{
  /*       struct mriq_device *mriq = to_mriq(esp); */
  /*    struct mriq_access *a = arg; */
...
static struct esp_driver mriq_driver = {
	...
	.ioctl_cm	= MRIQ_IOC_ACCESS,
	.arg_size	= sizeof(struct mriq_access),
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("mriq driver");

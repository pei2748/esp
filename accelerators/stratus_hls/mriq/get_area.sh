...
for dma_size in $DMA_SIZE; do
	...
	for paral in $PARAL; do
	    ...
	    AREA_TRACE="${PWD}/bdw_work/trace/hls.mriq.BASIC_P${paral}_A${arch}_DMA${dma_size}.s.trace"
	    AREA_LOG="${PWD}/area_P${paral}_A${arch}_DMA${dma_size}.log"
	    #	AREA_TRACE="${PWD}/bdw_work/trace/hls.mriq.BASIC_DMA${dma_size}.s.trace"
	    #	AREA_LOG="${PWD}/area_DMA${dma_size}.log"

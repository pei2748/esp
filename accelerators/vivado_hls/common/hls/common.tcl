...
foreach dma $dma_width {
	...
	foreach prl $prl_level {

	    # Create project
	    if {$datatype eq ""} {
		open_project "${ACCELERATOR}_dma${dma}_w${width}_p${prl}"
	    } else {
	    ...
	    add_files [glob ../src/*] -cflags "-I../inc -DDMA_SIZE=${dma} -DDATA_BITWIDTH=${width} -DPRL=${prl} -std=c++0x "
	    ...
	    # Config HLS
	    if {$datatype eq ""} {
		config_rtl -prefix "${ACCELERATOR}_dma${dma}_w${width}_p${prl}_"
	    } else {
